...
| `/collection` | Rangée de filtres de rareté : un `div` de six `button` portant leur couleur en style en ligne (`background-color: var(--color-rarity-l)30`). La même rangée existe sur `/global-collection`, avec un bouton "Liste de souhaits" en tête et sans espaces dans le style, et sur `/marketplace`. Grille : `div.flex.flex-wrap.justify-center.gap-3` dont chaque carte est enveloppée dans un `div.relative.isolate.group`, alors que `/global-collection` met la racine `glow-<rareté>` directement dans la grille (relevé le 2026-09-21). Pagination (confirmé par l'utilisateur). D'après les textes d'interface : tri (date d'ajout, nom, rareté, ATK, DEF, favoris), filtre par rareté, filtre par étiquette, gestion d'étiquettes y compris en lot sur une sélection, boutons Précédent et Suivant |
